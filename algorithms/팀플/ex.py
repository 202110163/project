    ...
    def __init__(self, title, times, age_limit):
        ...
        class ReservationSystem:
            def __init__(self, root):
                self.root = root
                self.users = {}
                self.load_user_data()  # 사용자 데이터 로드
...
class ReservationSystem:
    ...
    def save_user_data(self):
        with open("user_data.txt", "w") as file:
            for phone, password in self.users.items():
    ...
    def login(self):
        ...
        if phone_number in self.users and self.users[phone_number] == password:
            self.current_phone_number = phone_number
            self.main_menu()
        else:
            messagebox.showerror("오류", "전화번호 또는 비밀번호가 잘못되었습니다.")
